...
const char * xmd_boot_state_str[] = {"OFF", "FIRMWARE", "ON", "READY"};   
...
#endif  //#if defined(CONFIG_FLASHLESS_DEFAULT_BOOT_HSI)
extern int xmd_boot_hsi_drv_init(void);
extern void xmd_boot_hsi_drv_exit(void);
        ...
        if( (bool)value==(bool)hsi_drvmode ) {
            ...
            switch( value ) { //enum {XMD_HSI_DRVMODE_BOOT, XMD_HSI_DRVMODE_NORM, XMD_HSI_DRVMODE_NORM_SW,XMD_HSI_DRVMODE_NORM_HW};
                case XMD_HSI_DRVMODE_NORM:
                    pr_info("\nXMD: %s. xmd_boot_hsi_drv_exit start.\n",__func__);
                    xmd_boot_hsi_drv_exit();   /*unregister with xmd_boot HSI driver*/
                ...
                case XMD_HSI_DRVMODE_BOOT_SW:
                    ...
                    status = xmd_boot_hsi_drv_init();   /*Register with xmd_boot HSI driver*/
                    hsi_drvmode = value;    //hsi_drvmode, HSI驱动类型配置值: 0,NORMAL mode, >0,BOOT mode;
                    break;
...
static DEFINE_MUTEX(boot_flashless_mutex);  
